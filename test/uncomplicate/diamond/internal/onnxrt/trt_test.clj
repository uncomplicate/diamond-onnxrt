...
(ns ^{:author "Dragan Djuric"}
    uncomplicate.diamond.internal.onnxrt.trt-test
...
;;     (append-provider! opt :tensorrt) => opt))

;; (facts
;;   "Test session and session options."
;;   (let [env (environment nil)
;;         opt (-> (options) (append-provider! :tensorrt))
...
;; (facts
;;   "Test TensorRT memory-info."
;;   (with-release [env (environment nil)
;;                  opt (-> (options)
;;                          (append-provider! :tensorrt)
...
;;                  hstream (stream)
;;                  opt (-> (options)
;;                          (append-provider! :tensorrt {:stream hstream})
;;                          (severity! :verbose)
